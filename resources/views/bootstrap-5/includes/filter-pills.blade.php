<div>
    @if ($showFilters && count($this->getFiltersWithoutSearch()))
        <div class="mb-3">
            ...
            @foreach ($filters as $key => $value)
                @if ($key !== 'search' && filled($value))
                    <span wire:key="filter-pill-{{ $key }}"
                        ...
                        <a href="#" wire:click.prevent="removeFilter('{{ $key }}')" class="text-white ms-2">
                            <span class="visually-hidden">@lang('Remove filter option')</span>
                            <svg style="width:.5em;height:.5em;margin-top:3px;margin-right:3px" stroke="currentColor"
            ...
            <a href="#" wire:click.prevent="resetFilters" class="btn btn-sm btn-outline-secondary ml-2">
                @lang('Clear')
            </a>
        </div>
    @endif
